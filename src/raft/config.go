...
func (cfg *config) connect(i int) {
	PrettyDebug(dLog, "S%d connect", i)
...
func (cfg *config) disconnect(i int) {
	PrettyDebug(dLog, "S%d disconnect", i)
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		PrettyDebug(dLog, "expect one leader,leader info:%v", leaders)
...
func (cfg *config) checkNoLeader() {
	PrettyDebug(dLog, "expect no leader")
	for i := 0; i < cfg.n; i++ {
		if cfg.connected[i] {
